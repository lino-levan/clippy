...
import { createCanvas, loadImage, type CanvasRenderingContext2D } from "https://deno.land/x/canvas/mod.ts";
...
function calculateBubbleMetrics(
  ...
  const wrappedOptions = options.map(option => {
    const optionLines = wrapText(ctx, option, maxWidth - padding * 2 - 30, fontSize);
    return {
      lines: optionLines,
  ...
  const totalHeight = textHeight + optionsHeight + (options.length > 0 ? padding * 3 : padding * 2);
...
function drawClippyMessage(
  ...
  // Get bubble metrics
  const metrics = calculateBubbleMetrics(ctx, message, options, maxWidth, fontSize, padding);
  ...
  ctx.arcTo(
    bubbleX + metrics.width, bubbleY,
    bubbleX + metrics.width, bubbleY + cornerRadius,
    cornerRadius
  );
  ctx.lineTo(bubbleX + metrics.width, bubbleY + metrics.height - cornerRadius); // Right side to tail
  ctx.arcTo(
    bubbleX + metrics.width, bubbleY + metrics.height,
    bubbleX + metrics.width - cornerRadius, bubbleY + metrics.height,
    cornerRadius
  );
  ...
  ctx.arcTo(
    bubbleX, bubbleY + metrics.height,
    bubbleX, bubbleY + metrics.height - cornerRadius,
    cornerRadius
  );
  ...
  if (options.length > 0) {
    ...
    metrics.wrappedOptions.forEach((option, index) => {
      ...
      ctx.beginPath();
      ctx.arc(bubbleX + padding + 10, currentY - fontSize/2 + 2, 8, 0, Math.PI * 2);
...
export const handler: Handlers = {
  async GET(req: Request, _ctx: FreshContext) {
    ...
    const text = decodeURIComponent(url.searchParams.get("text") || "Hello! I'm Clippy!");
    ...
    const canvasHeight = Math.max(dimensions.height + padding + clippyHeight, clippyHeight + 40);
